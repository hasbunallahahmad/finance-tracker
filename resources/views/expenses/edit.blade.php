            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ route('expenses.update', $expense) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-input id="title" name="title" type="text" class="mt-1 block w-full"
                                value="{{ old('title', $expense->title) }}" required autofocus />
                            @error('title')
                        ...
                        <div>
                            ...
                            <x-input id="amount" name="amount" type="number" step="0.01" min="0"
                                class="mt-1 block w-full" value="{{ old('amount', $expense->amount) }}" required />
                            @error('amount')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="mt-1 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ old('description', $expense->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center justify-between mt-6">
                        <a href="{{ route('expenses.index') }}" class="text-gray-600 hover:text-gray-900">
                            {{ __('Kembali') }}
                        </a>
                        <x-button>
                            {{ __('Update') }}
